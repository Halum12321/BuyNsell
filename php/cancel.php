...
if ($resultMain->num_rows > 0) {
    if (is_array($rowMain)) {
        ...
        $sqlSIH = "UPDATE registered_product set STOCK = $new_stock where PRODUCT_CODE = '$pcodes'";
        $rsSIH = mysqli_query($con, $sqlSIH);
        if ($rsSIH) {
            $sqlSIH = "UPDATE purchase_basket set STATUS = 'Cancelled' where PRODUCT_CODE = '$pcodes'";
            $rsSIH = mysqli_query($con, $sqlSIH);
            if ($rsSIH) {
